 ...
 *
 */
package com.financia.exchange.im.handler;
...
public class UserReqHandler extends AbstractCmdHandler {
	...
	public Command command() {
		return Command.addAndGet("COMMAND_GET_USER_DIY_REQ",21);
	}
	...
	public ImPacket handler(ImPacket packet, ImChannelContext imChannelContext) throws ImException {
		...
		if(StringUtils.isEmpty(userId)) {
			return ProtocolManager.Converter.respPacket(new RespBody(Command.addAndGet("COMMAND_GET_USER_DIY_ERS",22), ImStatus.C10004), imChannelContext);
		}
		...
		RespBody resPacket = new RespBody(Command.addAndGet("COMMAND_GET_USER_DIY_ERS",22));
