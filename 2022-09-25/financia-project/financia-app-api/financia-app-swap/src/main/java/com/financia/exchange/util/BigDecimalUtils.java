...
public class BigDecimalUtils {
    ...
    /**
     * 对BigDecimal进行小数点后8位精度处理，并转换成字符串
     * @return
     */
    public static String getBigDecimalStatisticalProcess(BigDecimal bigDecimal){
        BigDecimal scale = bigDecimal.setScale(8, BigDecimal.ROUND_HALF_DOWN);
     ...
     * @return
     */
    public static BigDecimal getBigDecimal(BigDecimal totalRecharge, List<Object[]> list, Map<Object, Object> maps) {
        for(Object[] objects:list){
            BigDecimal amount=new BigDecimal(objects[1].toString());
            String unit=objects[0].toString();
            if(amount.compareTo(BigDecimal.ZERO)==1) {
                amount =amount.multiply(new BigDecimal(maps.get(unit).toString()));
