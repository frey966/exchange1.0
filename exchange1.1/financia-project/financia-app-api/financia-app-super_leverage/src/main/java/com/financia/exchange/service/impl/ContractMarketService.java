...
public class ContractMarketService {
    ...
    public List<KLine> findAllKLine(String symbol, long fromTime, long toTime, String period){
        ...
        List<KLine> kLines = mongoTemplate.find(query,KLine.class,"super_leverage_kilne_"+symbol.toUpperCase()+"_"+ period);
        return kLines;
    }
    ...
    public KLine saveKLine(String symbol, KLine kLine){
        ...
        if(queryKLine != null){
            ...
            mongoTemplate.updateFirst(query, update,"super_leverage_kilne_"+symbol+"_"+kLine.getPeriod());
            return queryKLine;
        }

//        logger.info("保存K线(" + symbol + "): " + kLine.getPeriod() + "/" + kLine.getTime() + "----maxTime: " + timeStamp);
        mongoTemplate.insert(kLine,"super_leverage_kilne_"+symbol+"_"+kLine.getPeriod());
        return kLine;
    }
    ...
    public KLine findMaxTimestampKLine(String symbol, String period, Long time) {
        ...
        List<KLine> result = mongoTemplate.find(query,KLine.class,"super_leverage_kilne_"+symbol+"_"+period);
