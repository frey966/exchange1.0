...
public abstract class JedisClusterTestBase {
  ...
  protected static final String LOCAL_IP = "54.179.184.125";

  @Before
  public void setUp() throws InterruptedException {
    node1 = new Jedis(nodeInfo1);
    node1.auth("chuanzhangredis");
    node1.flushAll();

    node2 = new Jedis(nodeInfo2);
    node2.auth("chuanzhangredis");
    node2.flushAll();

    node3 = new Jedis(nodeInfo3);
    node3.auth("chuanzhangredis");
    node3.flushAll();

    node4 = new Jedis(nodeInfo4);
    node4.auth("chuanzhangredis");
    node4.flushAll();

    nodeSlave2 = new Jedis(nodeInfoSlave2);
    nodeSlave2.auth("chuanzhangredis");
    nodeSlave2.flushAll();
    // ---- configure cluster
